...
const Model = {
  ...
  async fetchBurgers() {
    // If API_BASE is still the placeholder, fall back to local db.json for dev/live-server
    if (API_BASE.includes("YOUR-GITHUB-USERNAME") || API_BASE.includes("YOUR-REPO-NAME")) {
      const res = await fetch("./db.json");
      if (!res.ok) throw new Error("Could not load local db.json");
  ...
  async fetchSteps(burgerId) {
    if (API_BASE.includes("YOUR-GITHUB-USERNAME") || API_BASE.includes("YOUR-REPO-NAME")) {
      ...
      if (!res.ok) throw new Error("Could not load local db.json");
      const data = await res.json();
      return (data.steps || []).filter((s) => Number(s.burgerId) === Number(burgerId));
    }
